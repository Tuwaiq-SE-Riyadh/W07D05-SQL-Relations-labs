...
SELECT act_fname,act_lname, role  FROM movies.actor
 JOIN movie_cast ON actor.act_id=movie_cast.act_id 
 JOIN movie ON movie_cast.mov_id=movie.mov_id AND movie.mov_title='Annie Hall';
 
 SELECT act_fname,act_lname, mov_title, mov_dt_rel  FROM movies.actor
  JOIN movie_cast ON actor.act_id=movie_cast.act_id 
  JOIN movies.movie ON movie_cast.mov_id=movie.mov_id 
 where  NOT(mov_year>1990 and  mov_year < 2000) ;
 
 SELECT mov_year FROM movies.movie  JOIN movies.rating ON movie.mov_id = rating.mov_id 
 where rev_stars IN(3,4);
 
 SELECT mov_title, mov_dt_rel  FROM movies.movie WHERE mov_title LIKE "%Boogie Nights%";
 
 SELECT mov_title, dir_fname, dir_lname, rev_stars FROM movies.director
 JOIN movie_direction ON movie_direction.dir_id = director.dir_id 
 JOIN movie ON  movie_direction.mov_id = movie.mov_id 
 JOIN  rating ON movie_direction.mov_id = rating.mov_id
